...
# Routes that direct the applications views. 
...
# Register route
# Checks whether the user has two-factor authentication (2FA) enabled.
# If 2FA has not been set up, the user is prompted to register with
# an authenticator app before completing login and the registration
# form is rendered. Otherwise, the user is redirected to the home page.
...
#Similar to above however if the user isn't authenticated the login form is rendered
...
#Logs the user out and removes their ID from sessions and the user is redirected to the Login route
...
# The route for rendering the 2fa view, is triggered if user is not registered for 2fa in other routes
...
#2FA verfication
# This validates the OTP entered by the user
@accounts_bp.route("/verify-2fa", methods=["GET", "POST"])
@login_required
def verify_two_factor_auth():
    form = TwoFactorForm(request.form)
    if form.validate_on_submit():
        #Verifies OTP for current user
        if current_user.is_otp_valid(form.otp.data):
            ...
            #IF failed redirected to verify 2fa view to try again
            flash("Invalid OTP. Please try again.", "danger")
            return redirect(url_for(VERIFY_2FA_URL))
    else:
        #IF 2fa is not enabled user is asked to enable 2fa 
        if not current_user.is_two_factor_authentication_enabled:
            flash(
